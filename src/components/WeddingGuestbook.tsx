  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <section className="mt-5 rounded-2xl border border-sage-200/80 bg-white/75 p-5">
        ...
        {translationSettings.providers.length > 0 && <div className="mt-5 grid gap-2 sm:grid-cols-2">{translationSettings.providers.map((item) => <button key={item.provider} type="button" aria-label={`Edit ${item.provider}`} onClick={() => { setTranslationDraft(item); setTranslationModels([]); setTranslationKeys(''); }} className={`flex items-center justify-between rounded-xl border px-4 py-3 text-left transition ${item.provider === translationDraft.provider ? 'border-sage-600 bg-sage-50' : 'border-sage-200 bg-white hover:border-sage-400'}`}><span><span className="block font-montserrat text-[9px] font-bold uppercase tracking-[0.14em] text-wine-600">Priority {item.priority}</span><span className="mt-1 block font-serif text-base text-sage-900">{item.provider} · {item.model || 'Choose model'}</span></span><span className="text-right"><span className="block text-[10px] text-sage-500">{item.apiKeyCount} keys</span><span className="mt-1 block font-montserrat text-[9px] font-bold uppercase tracking-[0.12em] text-sage-700">Edit</span></span></button>)}</div>}
